...
RSpec.describe LoadMap::Targets do
  ...
  let(:targets) { described_class.build_from_file(svg_parser.result['LoadMap::Svg::Point'], xls_file_path) }

  it { expect(described_class.build_from_file(svg_parser.result['LoadMap::Svg::Point'], xls_file_path).xls_lines.count).to eq(5) }
  it { expect(described_class.build_from_file(svg_parser.result['LoadMap::Svg::Point'], xls_file_path).xls_lines[4]).to eq({ 'description' => 'Пиво, чипсы, телевизоры', 'id' => 'Point5', 'name' => '"Всё для футбола"', 'point_type' => 'shop' }) }
